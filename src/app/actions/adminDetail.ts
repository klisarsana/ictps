...
export async function getEmployeeDetail(userId: string): Promise<{ success: true; data: EmployeeDetailData } | { error: string }> {
  try {
    ...
    // Fetch user profile first to get the name
    const userResponse = await supabase.from("admin_dashboard_summary").select("user_id, name, email, role").eq("user_id", userId).limit(1).maybeSingle();
    ...
    // Fetch pemetaan and coaching concurrently
    const [pemetaanResponse, coachingResponse] = await Promise.all([
      supabase.from("pemetaan_diri").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(1).maybeSingle(),
      // Fetch coaching where this employee is the coachee
      supabase.from("coaching_records").select("*").ilike("nama_coachee", userProfile.name).order("created_at", { ascending: false }),
    ]);
...
export async function getCoachDetail(coachId: string): Promise<{ success: true; data: CoachDetailData } | { error: string }> {
  try {
    const supabase = await createClient();
    ...
    const [userResponse, coachingResponse] = await Promise.all([
      // Use the users table to ensure we find the coach even if they are not in the summary view
      supabase.from("users").select("id, name, email, role").eq("id", coachId).limit(1).maybeSingle(),
      // The coach created the record, so user_id = coachId
      supabase.from("coaching_records").select("*").eq("user_id", coachId).order("created_at", { ascending: false }),
    ]);

    if (userResponse.error) {
      console.error("Error fetching coach profile:", userResponse.error);
      return { error: "Failed to fetch coach profile." };
    }
    ...
    console.error("Unexpected error in getCoachDetail:", err);
    return { error: "An unexpected error occurred while fetching details." };
  }
}
